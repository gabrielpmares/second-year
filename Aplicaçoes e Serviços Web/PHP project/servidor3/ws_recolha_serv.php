...
{	
	...
	//	(SELECT cc FROM voluntario WHERE email='$idVol' AND dia='$diaInst')";
			...
			if(strcmp($dispVol, 'livre') == 0){
				...
				if(strcmp($dispInst, 'livre') == 0){
					array_push($s, "A instituição ainda tem o horário indicado livre <br> 
						            Aceite!");
				}// Se o voluntário escolheu uma instituição mas ela ainda não o aceitou
				elseif(strcmp($dispVol, 'livre') == 0 and strpos($dispInst, $idVol) !== false){
					array_push($s, "<br>O voluntário está á espera de ser aceite pela instiuição <br>
									Não aceite!");
				}
				// Se a instituição escolheu um voluntário mas ele ainda não aceitou
				elseif(strpos($dispVol, $idInst) !== false and strcmp($dispInst, 'livre') == 0){
					array_push($s, "<br>A instituição está à espera que o voluntário aceite <br> 
									Não aceite!");
				}
				else{
...
// Acrescentar mais coisas no array
